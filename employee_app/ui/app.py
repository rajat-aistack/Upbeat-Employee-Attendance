...
class AttendanceApp(ctk.CTk):
    ...
    def _apply_status(self, status: dict):
        """Apply today's status to the UI."""
    ...
    def _do_punch_in(self):
        ...
        def _action():
            ...
            try:
                if check_internet_connectivity() and self.api_client.check_health():
                    result = self.api_client.punch_in(payload)
                    ...
                    self.after(0, lambda: show_toast(
                        self, "Saved offline — will sync automatically", "warning"
                    ))

            except APIError as e:
                self.after(0, lambda: self.current_view.set_error(e.detail))
            except Exception as e:
                # Store offline on any error
                self.offline_store.store_record(
                    action="punch_in",
                    device_fingerprint=self.fingerprint,
                    hostname=payload["hostname"],
                ...
                self.after(0, lambda: self.current_view.set_checked_in(time_str))
                self.after(0, lambda: show_toast(
                    self, "Saved offline — will sync when connected", "warning"
                ))
    ...
    def _do_punch_out(self):
        ...
        def _action():
            ...
            try:
                if check_internet_connectivity() and self.api_client.check_health():
                    result = self.api_client.punch_out(payload)
                    time_str = now.strftime("%I:%M %p")
                    self.after(0, lambda: self.current_view.set_checked_out(time_str))
                else:
                    self.offline_store.store_record(
                        action="punch_out",
                        device_fingerprint=self.fingerprint,
